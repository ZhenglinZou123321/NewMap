...
def update_cav_speeds(intersection_id,traffic_light_to_lanes,last_quarter_vehicles,N,dt,L_safe):
    ...
    for lane_id in traffic_light_to_lanes[intersection_id]:
        ...
        if len(initial_state_CAV) != 0:
            ...
            control_signal = QP_solver(initial_state_CAV, initial_state_HDV, vehicles_list_this_lane, N, dt, v_max=MAX_SPEED, v_min=MIN_SPEED, a_max=MAX_ACCEL,a_min=MIN_ACCEL, L_safe=L_safe, lane_now=lane_id, CAV_id_list=CAV_id_list, HDV_id_list=HDV_id_list,control_signal=control_signal)
